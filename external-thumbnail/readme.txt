=== External Thumbnail ===
Contributors: mrtaiw
Contributor's website: http://thucdem.mobi
Donate link: http://thucdem.mobi
Tags: external thumbnail, external featured image, remote thumbnail, thumbnail via url
Requires at least: 3.3
Tested up to: 4.5.2
Stable tag: trunk

Using external images from anywhere to make thumbnail

== Description ==

Using external images from anywhere to make thumbnail.

If empty url image, will take the first picture in the post as thumbnail

== Installation ==

1. Upload `external-thumbnail.zip` to the `/wp-content/plugins/` directory and Unzip
2. Activate the plugin through the 'Plugins' menu in WordPress

== Screenshots ==

1. Add external thumbnail

== Support ==

Having problems or need support? Feel free to email mrtaiw@gmail.com or Facebook https://www.facebook.com/taiw96

== Changelog ==

*1.01*
Fix override additional image

*1.1.1*
Fix override additional image if attr is null

*1.2*
Fix not delete field external thumbnail
Optimal first image filtering
*1.2.1*
Preview thumbnail


